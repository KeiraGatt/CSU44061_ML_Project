...
# & find best weight for L2 Penalty
...
def l2():
    ...
    # Get list of features to remove by
    # applying chi-square test
    features_idx = feature_selection()
    features = features.drop(features.columns[features_idx], axis=1)
    ...
    for d in [x for x in range(max_deg) if x != 0]:
        ...
        for c in c_range:
            ...
            model = LogisticRegression(penalty='l2', C=c, solver='liblinear', max_iter=10000)
    ...
    plt.title('L2 - Mean and Standard Deviation of the F1 Score vs C')
    plt.legend()
    #plt.savefig('L2- Cross-Val')
    plt.show()
